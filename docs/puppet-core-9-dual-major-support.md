...
**Status:** Implemented — Steps A–E shipped (E landed via PR #19, see §12)
...
- **Puppet 9.0.0's facter pin is an assumption, not a lookup.** Step D pinned
  `9-latest-maintained` to `facter_version: 4.21.0` — the newest facter build
  known to exist on the private source (it is what `8-latest-maintained` uses,
  and public rubygems.org tops out at 4.10.0, so 4.x above that is
  Puppet-Core-only). It satisfies Puppet 9.0.0's declared `facter >= 4.3.0, < 5`
  requirement from §8's gemspec query. What was *not* verifiable without an API
  key is whether the private source ships a facter release specifically paired
  with 9.0.0, and whether Puppet 9's code needs anything newer than 4.21.0 at
  runtime. `bootstrap.rb` pins facter exactly (`gem 'facter', '= <version>'`),
  so a wrong pin surfaces immediately as a bundler resolution failure in the
  gate dispatch, not as a subtle runtime bug — cheap to detect, cheap to bump.
- **Acceptance on major 9 depends on a `puppet9` package collection existing.**
  `docker.rb` derives everything from the profile's `puppet_major`
  (`collection = "puppet#{puppet_major}"`), so the Docker build will reach for
  `puppet9-release-*` on `yum-puppetcore` / `apt-puppetcore`. Nothing in the
  harness needs changing if those exist; if they don't, major 9 acceptance
  fails at image build while the unit lane is unaffected. Related but benign:
  `config/compat-rpms/openvox-server.spec` is versioned `8.0.0`, but that is the
  shim package's own version and it only declares `Requires: puppetserver` — the
  actual puppetserver comes from whichever collection the run installed, so it
  does not pin major 8 into a Puppet 9 run.
...
| C | Self-contained Puppet-8 caller + shared composite actions | **Done — closed out 2026-08-21** | **First attempt (reverted):** built `.github/workflows/_compatibility-runner-reusable.yml` (`workflow_call`, held the full `prepare`/`test_unit`/`test_acceptance`/`publish` graph) + thin `compatibility-runner-puppet8.yml` caller, per the original §4.1 plan, on branch `puppet9-step-c-ci-topology` / PR [#16](https://github.com/puppetlabs/puppet-module-compat-harness/pull/16) (merged). Hit a GitHub platform constraint first (workflow_dispatch on a brand-new workflow file is only discoverable once the file exists on the default branch, even with `--ref` pointed at a branch — never came up in Steps A/B since those only edited files already on `main`); PR #16 was merged by the user for that reason. Live dispatch after merge then surfaced a real regression the constraint-check hadn't: the Actions run page's **left sidebar** collapsed every unit-test job's name down to just `unit` (identical across all of them), even though the Jobs API, Checks API, and the job detail header all showed the correct, unique name (`test / <module> / unit`) the whole time. A renaming fix (matching the caller job's name to the pre-existing first name segment, to avoid an extra nesting level) did NOT fix it — proven by re-checking the same "fixed" run's sidebar. **Root cause, isolated via a controlled A/B dispatch** (identical job/matrix/name YAML; only the `workflow_call` boundary itself differed): GitHub cannot correctly render a job's `strategy.matrix`-derived dynamic name in that sidebar specifically when the job is defined inside a workflow invoked via `workflow_call` — a platform limitation with no YAML-level workaround. A suggestion (from a second agent consulted by the user) to pass the parent job's name into the child as an input doesn't apply: it only disambiguates multiple *static* calls to a reusable workflow, not one job that internally fans out via a matrix — the matrix expansion itself is what's inside the broken boundary. **Fix shipped:** dropped the reusable workflow entirely; `compatibility-runner-puppet8.yml` now defines all four jobs directly (no `workflow_call`), and DRY is achieved at the *step* level instead via two new composite actions — `.github/actions/prepare-test-matrix` and `.github/actions/publish-compatibility-results` — which never create a separate job/check-run and so never hit this bug (same reason the pre-existing `.github/actions/run-module-test` composite action, shared by both matrix jobs since before Step C, was never affected). `detect_changes.py`'s material-path scoping (§5.3) is unaffected by any of this churn — `SHARED_MATERIAL_PATHS` now points at `.github/actions` (covering all three composite actions) plus a `CALLER_WORKFLOW_FILE` env var for the caller's own file. Verified via three live dispatches on `main`: two unit-only smoke tests (`puppet-boolean`, `puppet-autofs`) and one including an acceptance target (`puppet-telegraf`, `el9-systemd`) — all green, all with correct sidebar names confirmed by the user directly. Branch `puppet9-step-c-retire-old-workflow` (prepared, not yet merged) does the swap-over: adds the `schedule:` cron to the new caller and deletes the old `compatibility-runner.yml` in one commit, plus doc reference updates (`AGENTS.md`/`CLAUDE.md`/`CONTRIBUTING.md`/`README.md`/this file). **Closed out 2026-08-21:** the user ran the full-suite dispatch and merged the swap-over branch to `main` via PR [#17](https://github.com/puppetlabs/puppet-module-compat-harness/pull/17), so `compatibility-runner.yml` is gone and `compatibility-runner-puppet8.yml` owns the nightly cron. |
| D | Puppet 9 profile + caller | **Done — gate passed 2026-08-21** | **Pin `9-latest-maintained` to `puppet_core_version: 9.0.0` explicitly — decided 2026-08-21, do NOT re-query the gem source for a newer 9.x for this first cut** (supersedes this doc's earlier guidance to always re-check; that guidance was written before the product decision to deliberately start at `9.0.0`). **As implemented:** `9-latest-maintained` = Puppet 9.0.0 / facter 4.21.0 / bundler 2.5.22 / private source; no `9-previous-maintained` (9.0.0 is the first 9.x — there is no previous to pin, so that entry waits for 9.1). `classify_module_result.py` now stamps `puppet_major` from the profile (the field `update_ledger.py` has been keying on since Step B, previously always defaulting to 8). `detect_changes.py` takes the major from a `PUPPET_MAJOR` env var, surfaced as the `puppet-major` input on `prepare-test-matrix` and set explicitly by both callers ('8' / '9'). `compatibility-runner-puppet9.yml` gained its own nightly cron at 03:00 UTC (an hour after the Puppet 8 caller, to avoid runner contention — not for correctness; the shared `compat-ledger-*` group already serializes the only racing step). `build_matrix.rb` needed **no** change despite §10 listing it: each caller picks its major purely via the `profile` it passes to `run-module-test`, and per-major leanness arrives pre-resolved in `INCLUDE_IDS`, so matrix rows are major-agnostic — a comment in the file now says so, to stop the next reader looking for missing plumbing. Local pre-gate checks against the real ledger: `detect_changes.py` with `PUPPET_MAJOR=9` includes all 75 modules (`never-tested`) while `PUPPET_MAJOR=8` includes 0 (all green/fresh); after merging a synthetic Puppet 9 run (one clean, one failing) into a ledger copy, the same pair returns 74 included on 9 (73 never-tested + 1 `unit-failing`) and still **0** on 8 — §5's independence requirement demonstrated locally, with `puppet-telegraf` simultaneously `unit+acceptance` on 8 and `unit-failing` on 9 in one row, and the dashboard rendering both majors' column pairs off that ledger. **Ruby version cycling before the gate closed:** the profile's `ruby_version` started at 3.4 (the floor `puppet 9.0.0`'s gemspec requires), was bumped to 4.0 to match what most real users will actually run against Puppet Core 9, then compared against a Ruby 3.4 run head-to-head (same commit range) to isolate Ruby-version-caused differences from real Puppet 9 incompatibilities. The comparison found: (a) 9 modules/lanes fail identically on both Rubies — real declared-metadata Puppet 9 ceilings (`puppet-hiera`/`puppet-corosync`/`puppet-collectd` acceptance; `puppet-gitlab`/`puppet-extlib`/`puppet-windows_env`/`tragiccode-azure_key_vault`/`suchpuppet-puppet-resolvconf`/`stschulte-puppet-oracle` unit); (b) 8 additional lanes fail **only** under Ruby 4.0 (`saz-puppet-limits`/`saz-puppet-memcached`/`saz-puppet-sudo`/`saz-puppet-timezone` unit+acceptance, `puppet-boolean`/`smoeding-puppet-debconf` unit) — not real incompatibilities but a harness dependency-resolution failure: `puppet_metadata` and `voxpupuli-acceptance` (pulled in via `Gemfile.puppetcore`) both cap their gemspecs at `Ruby < 4`, so Bundler can't resolve at all under 4.0.6 (`"current Ruby version is = 4.0.6, version solving has failed"`); (c) 2 lanes failed **only** under Ruby 3.4 and were unrelated CI/Ruby-3.4 flakiness, not Puppet 9 signal — `puppet-wget` unit hit a Ruby-3.4-specific bundler default-gem clash (`"You have already activated ostruct 0.6.1, but your Gemfile requires ostruct 0.6.3"`), and `puppet-nfs` acceptance had all 13 examples pass with the only failure being a Docker container-cleanup race in an `after(:suite)` hook, unrelated to Ruby or Puppet. **Decision (2026-08-21):** keep `ruby_version: "4.0"` — Perforce and Vox Pupuli are both still mid-migration to Puppet 9, so it's expected that some modules' own tooling gems aren't Ruby-4-ready yet; matching the Ruby version real users will actually run is more valuable than hiding that gap behind an older Ruby. Those 8 lanes will keep showing `harness_error` until upstream ships Ruby-4-compatible `puppet_metadata`/`voxpupuli-acceptance` releases. **Gate result:** Puppet 9 caller dispatched full suite under Ruby 4.0 with the publish-persist fix (§ below) — 12 unit failures / 7 acceptance failures, matching the root-cause breakdown above exactly (run [32483852382](https://github.com/puppetlabs/puppet-module-compat-harness/actions/runs/32483852382)). Puppet 8 caller re-dispatched in the same round (run [32494370653](https://github.com/puppetlabs/puppet-module-compat-harness/actions/runs/32494370653)) — full suite (the 48h harness-change window forced `run_all`, since `profiles/puppet_profiles.json` had just been touched), came back **fully green with zero compatibility-state changes**: the resulting commit ([7fcc290](https://github.com/puppetlabs/puppet-module-compat-harness/commit/7fcc290)) only touched `STATUS.md`'s timestamp line and `status/ledger.json`'s per-module `tested_at`/`last_run_id` stamps — `KNOWN_COMPATIBLE.md` was regenerated identically and produced no diff, since no Puppet 8 module's compatibility symbol changed. This is the strongest possible confirmation of §5's independence requirement: a full re-test of all 75 modules against Puppet 8 was completely unaffected by all the Puppet 9 Ruby-version churn. **Also fixed during this step:** `publish-compatibility-results`'s summarize step previously short-circuited the ledger-update/dashboard-render/git-commit steps whenever any module failed, so a red run silently never persisted its results — fixed so those steps always run regardless of the summarize step's own (informational, per-module) failure annotations; the publish job can still show red in the Actions UI when modules fail, but that's now cosmetic. |
| E | `KNOWN_INCOMPATIBLE.md` policy + docs | **Done — doc/policy edits landed 2026-08-21, no dispatch gate needed (§9)** | `AGENTS.md`'s "Adding New Incompatibilities" section now states the §7 removal scope explicitly: a removal (KNOWN_INCOMPATIBLE.md entry + `config/modules.json` deletion) only happens when the module is incompatible on Puppet 8 (gating) or on every actively tested major; a non-gating-major-only incompatibility (e.g. Puppet-9-only) is not a removal event and needs no special handling beyond the per-major red cell the dashboard already renders. The `mark-incompatible` skill got the same nuance folded into its Step 0 classification (a new "Which major(s)?" check ahead of the existing Incompatible/Partial/Deprecated split) and a note on the `KNOWN_INCOMPATIBLE.md` "Puppet Core Tested" column recording multiple majors when relevant. `docs/architecture-flow.md` gained a new intro paragraph describing the two-self-contained-caller topology (with the `workflow_call` sidebar-bug rationale carried over from §4/§12's Step C entry), plus per-major detail in the "Detect changes," "Record module status," and "CI: Publish" rows/prose (`PUPPET_MAJOR`-scoped ledger reads and material-path scoping, `puppet_major` in `module-status.json`, `update_ledger.py` grouping by `(id, major)`, per-major `is_fully_compatible` and column pairs in `STATUS.md`/`KNOWN_COMPATIBLE.md`) — verified against the actual shipped code (`detect_changes.py`, `render_status_dashboard.py`, `update_ledger.py`) rather than restated from this design doc, since Steps B–D had already shipped ahead of the diagram doc catching up. Landed via branch `puppet9-step-e-incompatibility-policy` / PR [#19](https://github.com/puppetlabs/puppet-module-compat-harness/pull/19). No dispatch gate applies here (§9: doc/policy-only) — merge of this PR is the only remaining condition, and this row is written as already-Done so `main` reflects the correct state the moment it merges, with no separate follow-up commit needed. |

**Baseline context (as of 2026-08-20):**
- `profiles/puppet_profiles.json`: `9-latest-maintained` = Puppet 9.0.0 / facter 4.21.0 / Ruby 4.0 (added in Step D, 2026-08-21; started at Ruby 3.4, settled on 4.0 after the head-to-head comparison described in Step D's row above); `8-latest-maintained` = Puppet 8.21.0 / facter 4.21.0; `8-previous-maintained` = Puppet 8.20.0 / facter 4.20.0 (bumped in a prior session alongside the Puppet 9 investigation — a new Puppet 8.x point release shipped alongside Puppet 9). Full-suite baseline under these confirmed green (no reds) before Step A's first attempt.
