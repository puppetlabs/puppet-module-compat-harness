...
**Status:** In progress — Steps A–C shipped on `main`; Step D implemented, awaiting its gate dispatch; Step E not started (see §12)
...
- `profiles/puppet_profiles.json`: `9-latest-maintained` = Puppet 9.0.0 / facter 4.21.0 / Ruby 3.4 (added in Step D, 2026-08-21); `8-latest-maintained` = Puppet 8.21.0 / facter 4.21.0; `8-previous-maintained` = Puppet 8.20.0 / facter 4.20.0 (bumped in a prior session alongside the Puppet 9 investigation — a new Puppet 8.x point release shipped alongside Puppet 9). Full-suite baseline under these confirmed green (no reds) before Step A's first attempt.
