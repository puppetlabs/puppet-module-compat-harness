  ...
  class Adapters
    ...
    def collect_acceptance_debug(module_dir, env)
      ...
      debug_script = <<~'SH'
        ...
        if [ -n "$IMAGE" ]; then
          echo "Image: $IMAGE"
          echo "Note: systemd will log to console, then timeout and container stops cleanly."
          docker run --rm --privileged \
            --tmpfs /run --tmpfs /run/lock \
            -e container=docker \
            "$IMAGE" \
            timeout 15 /sbin/init --log-target=console --log-level=debug || EXIT_CODE=$? ; echo "init timeout/exit: ${EXIT_CODE:-0}" 2>&1
        else
